  ...
  return (
    <div style={{
      ...
      <div style={{ animation:"fadeUp 0.5s 0.5s ease both", opacity:0, width:120, height:2, background:"rgba(0,210,180,0.15)", borderRadius:2, overflow:"hidden", position:"relative", zIndex:1, marginTop:8 }}>
        <div style={{ height:"100%", background:"linear-gradient(90deg,#00d2b4,#0066ff)", borderRadius:2, animation:"barFill 2.2s ease both" }} />
      </div>
...
const AddMoneyScreen = ({ onBack, onSuccess, wallet }) => {
  ...
  return (
    <div style={{ background: "#F9F8FF" }}>
      ...
      {step === 3 ? (
        <div style={{ padding: "10px 13px", textAlign: "center", animation: "fadeUp 0.4s ease" }}>
          <div style={{ fontSize: 36, marginBottom: 16 }}>🎉</div>
          <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 16, fontWeight: 800, color: C.accent }}>Money Added!</div>
          <div style={{ color: C.muted, marginTop: 8, marginBottom: 8 }}>₹{credited.toLocaleString("en-IN")} credited to wallet</div>
          <div style={{ fontSize: 11, color: C.muted, marginBottom: 32 }}>Platform fee ₹{fee} deducted</div>
          ...
          <SwipeBtn onSubmit={() => onSuccess(credited, fee, method)} label="Swipe to Go to Dashboard" successLabel="🏠 Going Home..." color1="#059669" color2="#0891B2" />
          <button onClick={() => onSuccess(credited, fee, method)} style={{ width:"100%", marginTop:10, background:"none", border:"1.5px solid rgba(5,150,105,0.30)", borderRadius:12, padding:"11px", fontSize:11, fontWeight:700, color:"#059669", cursor:"pointer" }}>
            ← Back to Dashboard
          </button>
        </div>
      ) : step === 2 ? (
        <div style={{ padding: "20px 16px", textAlign: "center" }}>
          <Loader size={48} />
          <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 14, fontWeight: 700, marginTop: 24 }}>Processing Payment</div>
          <div style={{ color: C.muted, fontSize: 12, marginTop: 8 }}>Please wait, contacting bank...</div>
          <div style={{ marginTop: 24, display: "flex", flexDirection: "column", gap: 12 }}>
            {["Initiating payment request", "Card network approval", "Confirming transaction"].map((s, i) => (
              <div key={s} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 16px", background: "#EDE9FE", borderRadius: 10 }}>
                <Loader size={14} color={C.accent} />
    ...
    <div style={{ background: "#F9F8FF" }}>
      ...
      <div style={{ padding: "9px 11px", overflowY: "auto" }}>
        {step === 2 ? (
          <div style={{ textAlign: "center", animation: "fadeUp 0.4s ease" }}>
            <div style={{ fontSize: 36, marginBottom: 16 }}>✅</div>
            <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 16, fontWeight: 800, color: C.accent }}>Transfer Initiated!</div>
            <div style={{ color: C.muted, marginTop: 8, marginBottom: 24 }}>₹{net.toLocaleString("en-IN")} will credit to your bank shortly</div>
            <Card glow style={{ marginBottom: 24, textAlign: "left" }}>
              {[["Transfer Amount", fmt(Number(amount))], ["Transfer Fee", `− ${fmt(FEE)}`], ["Net Credit", fmt(net)], ["Mode", mode.toUpperCase()], mode==="upi" ? ["UPI VPA", upiVpa] : ["To Account", bank.accNo?.replace(/.(?=.{4})/g, "•") || "•••• 5678"], ["ETA", modes.find(m => m.id === mode)?.time]].map(([k, v]) => (
                <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: `1px solid ${C.border}` }}>
                  <span style={{ color: C.muted, fontSize: 12 }}>{k}</span>
                  <span style={{ fontWeight: 600, fontSize: 12, color: k === "Net Credit" ? C.accent : C.text }}>{v}</span>
                </div>
              ))}
            </Card>
            <SwipeBtn onSubmit={() => onSuccess(Number(amount), FEE, mode)} label="Swipe to Go to Dashboard" successLabel="🏠 Going Home..." color1="#059669" color2="#0891B2" />
              <button onClick={() => onSuccess(Number(amount), FEE, mode)} style={{ width:"100%", marginTop:10, background:"none", border:"1.5px solid rgba(5,150,105,0.30)", borderRadius:12, padding:"11px", fontSize:11, fontWeight:700, color:"#059669", cursor:"pointer" }}>
                ← Back to Dashboard
              </button>
          </div>
        ) : step === 1 ? (
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:8 }}>
                      <div style={{ flex:1 }}>
                        <div style={{ fontFamily:"'Space Grotesk',sans-serif", fontWeight:900, fontSize:10, color:"#1A1035" }}>
                          ₹{Number(b.amount).toLocaleString("en-IN")} · {b.cashMode==="hand"?"By Hand":b.cashMode==="neft"?"NEFT":"UPI"}
                        </div>
...
const RoleLoginScreen = ({ onLogin, agentDocs }) => {
  ...
  const BACK_MAP = {
    agentChoice:"roles", agentLogin:"agentChoice", agentIdLogin:"agentChoice", adminChoice:"roles", adminMobileLogin:"adminChoice", adminOtp:"adminChoice",
    agentRegister:"agentChoice", agentOtp:"agentChoice", awaitingApproval:"agentChoice", admin:"roles",
    doorstep:"doorstepHome", doorstepHome:"roles", doorstepLogin:"roles", doorstepLoginOtp:"doorstepLogin", doorstepForm:"doorstep", doorstepBooking:"doorstepForm", customerTrack:"doorstepHome", customerOtp:"doorstepHome",
  };
  ...
  return (
    <div style={{ height:"100%", display:"flex", flexDirection:"column", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", flex:1 }}>
          ...
          <div style={{ background:"linear-gradient(135deg,#6D28D9,#0891B2)", padding:"16px 16px 20px", position:"relative", overflow:"hidden", flexShrink:0, transform:"translateZ(0)" }}>
            ...
            <div style={{ textAlign:"center" }}>
              ...
              <div style={{ fontSize:9, color:"rgba(255,255,255,0.70)", letterSpacing:"0.05em" }}>Select your role to continue</div>
            </div>
            {/* Decorative orbs */}
          ...
          <div style={{ position:"relative", zIndex:1, display:"flex", flexDirection:"column", flex:1, justifyContent:"center", alignItems:"center" }}>
            ...
            <div style={{ width:"100%", padding:"0 20px", display:"flex", flexDirection:"column", gap:10 }}>
              ...
              <button onClick={() => { setRole("admin"); setView("adminChoice"); }} style={{
                background:"linear-gradient(135deg,#6D28D9,#9B5FFF)",
                ...
                <div style={{ width:38, height:38, borderRadius:11, background:"rgba(255,255,255,0.18)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, position:"relative" }}>
                  {/* Admin icon: shield with person + star badge */}
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                    {/* Shield body */}
                    <path d="M12 2L4 5.5V11c0 4.5 3.5 8.2 8 9 4.5-.8 8-4.5 8-9V5.5L12 2z"
                      fill="#fff" fillOpacity="0.22" stroke="#fff" strokeWidth="1.5" strokeLinejoin="round"/>
                    ...
                    <path d="M18.5 3.5l.5 1.5h1.5l-1.2 1 .4 1.5-1.2-.9-1.2.9.4-1.5-1.2-1H18z"
                      fill="#fff" stroke="#fff" strokeWidth="0.3" strokeLinejoin="round"/>
                  </svg>
                </div>
                <div style={{ flex:1 }}>
              ...
              <button onClick={() => { setRole("agent"); setView("agentChoice"); }} style={{
                ...
                animation:"fadeUp 0.4s 0.1s ease both", position:"relative", overflow:"hidden",
              }}>
                <div style={{ width:38, height:38, borderRadius:11, background:"rgba(255,255,255,0.18)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, position:"relative" }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
      ...
      {view === "doorstepHome" && (
        <div style={{ display:"flex", flexDirection:"column", height:"100%", animation:"slideIn 0.35s ease both" }}>
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"16px 14px 20px" }}>
            <p style={{ fontSize:9, color:C.muted, marginBottom:16, lineHeight:1.6 }}>What would you like to do?</p>
            ...
            <button onClick={() => { setTrackMobile(dsLoginMobile); setView("customerTrack"); }}
              style={{ width:"100%", display:"flex", alignItems:"center", gap:12, padding:"14px 16px", background:"rgba(8,145,178,0.07)", border:"1.5px solid rgba(8,145,178,0.30)", borderRadius:16, cursor:"pointer", textAlign:"left", boxShadow:"0 3px 12px rgba(8,145,178,0.10)", animation:"fadeUp 0.3s 0.1s ease both" }}>
            ...
            {/* Info */}
            <div style={{ marginTop:16, padding:"10px 12px", background:"rgba(5,150,105,0.05)", borderRadius:10, border:"1px solid rgba(5,150,105,0.12)" }}>
              <div style={{ fontSize:8, color:"#059669", fontWeight:900, marginBottom:4 }}>💡 How it works</div>
              <div style={{ fontSize:8, color:C.muted, lineHeight:1.7 }}>1. Book a withdrawal → provide your details & amount<br/>2. Admin assigns a nearby agent to your request<br/>3. Agent arrives within 30-60 mins & hands over cash</div>
            </div>
          </div>
        </div>
      )}
      ...
      {view === "roles" && (
        <div style={{ flexShrink:0, zIndex:10, background:"transparent" }}>
          <PaymentLogosStrip theme="light" size="xxxs" />
        </div>
      )}
    </div>
    ...
    <div style={{ animation:"slideIn 0.3s ease both" }}>
      ...
      <div style={{ background:"#fff", borderRadius:14, border:"1px solid rgba(0,0,0,0.07)", padding:"12px 14px", marginBottom:10, boxShadow:"0 2px 10px rgba(0,0,0,0.05)" }}>
        ...
        {[
          ...
          { iconName:"rupee",     color:"#D97706", label:"Amount",      value:"₹"+Number(selBooking.amount).toLocaleString("en-IN") },
          ...
          { iconName:"phone",     color:"#059669", label:"Called",      value:selBooking.agentCalled?"Yes ✅":"No" },
          { iconName:"kyc",       color:"#059669", label:"Approved At", value:selBooking.agentApprovedAt||"—" },
          { iconName:"send",      color:"#0891B2", label:"Transfer Done",value:selBooking.transferDoneAt||"—" },
